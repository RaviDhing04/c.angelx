...
export const getLatestProductsWithPagination = (body = {}) => async (dispatch) =>{
    try {
        const response = await httpFetch(getApiEndPoints('LatestProductsWithPagination'), {
            method: "POST",
            body: body
        });
...
export const getSelectedProductDetails = (body = {}) => async (dispatch) =>{
    try {
        const response = await httpFetch(getApiEndPoints('SelectedProductDetails'), {
            method: "POST",
            body: body
        });
...
export const getCartItems = (body = {}) => async (dispatch) =>{
    try {
        ...
        if(response && response.result && response.result.data && response.result.message === "Success" ) {
          const cartProducts = makeCartProduct(response.result.data.cartDetails.Items, response.result.data.productDetails.Items)
          dispatch({
            type: 'CART_DETAILS',
  ...
  cartItems.forEach((item) => {
     if (item.ProductId.S === productId) {
       if(+item.Quantity.S > 1) {
        updatedCartItem.push({...item, Quantity: {S: ((+item.Quantity.S)-1).toString()}})
       }
     } else {
...
export const deleteAllProductFromCart = () => (dispatch, getState) => {
  dispatch({
   type: 'CART_DETAILS',
   value: { payload : [] }
 });
