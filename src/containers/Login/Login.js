...
const Login = props => {
  ...
  const loginUser = async event => {
    ...
    res ? (res === 'reset' ? setLoading(false) : res === 'success' ? (function () { setLoading(false); props.location.pathname.includes('landing') ? (localStorage.getItem('startSelling') ? (function () { localStorage.removeItem('startSelling'); props.history.replace(`/manageBusiness/${JSON.parse(localStorage.getItem('userData')) && JSON.parse(localStorage.getItem('userData')).UserId}`); }()) : props.history.replace('/home/productsListing')) : props.history.replace(props.location.pathname) }()) : setLoading(false)) : (function () { setLoading(false); (alert('something went wrong, Please try again!')) }());
  };
